"""Tests for mlpstamps's output directory customization feature."""
...
def template(tmp_path):
    ...
    template_dir.mkdir()
    template_dir.joinpath('mlpstamps.json').touch()
    return str(template_dir)


@pytest.fixture(autouse=True)
def mock_gen_context(mocker, context) -> None:
    """Fixture. Automatically mock mlpstamps's function with expected output."""
    mocker.patch('mlpstamps.main.generate_context', return_value=context)


@pytest.fixture(autouse=True)
def mock_prompt(mocker) -> None:
    """Fixture. Automatically mock mlpstamps's function with expected output."""
    mocker.patch('mlpstamps.main.prompt_for_config')
...
def test_api_invocation(mocker, template, output_dir, context) -> None:
    ...
    main.mlpstamps(template, output_dir=output_dir)

...
def test_clone_should_raise_if_vcs_not_installed(mocker, clone_dir) -> None:
    ...
    is installed."""
    mocker.patch('mlpstamps.vcs.is_vcs_installed', autospec=True, return_value=False)

    repo_url = 'https://github.com/pytest-dev/mlpstamps-pytest-plugin.git'
...
def test_clone_should_rstrip_trailing_slash_in_repo_url(mocker, clone_dir) -> None:
    ...
    present."""
    mocker.patch('mlpstamps.vcs.is_vcs_installed', autospec=True, return_value=True)

    mock_subprocess = mocker.patch(
        'mlpstamps.vcs.subprocess.check_output',
        autospec=True,
    )
...
def test_clone_should_abort_if_user_does_not_want_to_reclone(mocker, clone_dir) -> None:
    ...
    mocker.patch('mlpstamps.vcs.is_vcs_installed', autospec=True, return_value=True)
    mocker.patch(
        'mlpstamps.vcs.prompt_and_delete', side_effect=SystemExit, autospec=True
    )
    mock_subprocess = mocker.patch(
        'mlpstamps.vcs.subprocess.check_output',
        autospec=True,
    ...
    repo_dir = clone_dir.joinpath('mlpstamps-pytest-plugin')
    repo_dir.mkdir()
...
def test_clone_should_silent_exit_if_ok_to_reuse(mocker, tmpdir) -> None:
    ...
    mocker.patch('mlpstamps.vcs.is_vcs_installed', autospec=True, return_value=True)
    mocker.patch(
        'mlpstamps.vcs.prompt_and_delete', return_value=False, autospec=True
    )
    mock_subprocess = mocker.patch(
        'mlpstamps.vcs.subprocess.check_output',
        autospec=True,
    ...
    # Create repo_dir to trigger prompt_and_delete
    clone_to_dir.mkdir('mlpstamps-pytest-plugin')
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('git', 'git@gitlab.com:mlpstamps/mlpstamps.git', 'mlpstamps'),
        ('git', 'git@github.com:mlpstamps/mlpstamps.git', 'mlpstamps'),
    ],
)
...
def test_clone_handles_branch_typo(mocker, clone_dir, error_message) -> None:
    ...
    repository_url = 'https://github.com/pytest-dev/mlpstamps-pytest-plugin'
...
def test_clone_unknown_subprocess_error(mocker, clone_dir) -> None:
    ...
    with pytest.raises(subprocess.CalledProcessError):
        vcs.clone(
            'https://github.com/pytest-dev/mlpstamps-pytest-plugin',
            clone_to_dir=str(clone_dir),
            no_input=True,
        )

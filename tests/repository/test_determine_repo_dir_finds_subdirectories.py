"""Tests around locally cached mlpstamps template repositories."""
...
def cloned_mlpstamps_path(user_config_data, template):
    ...
    Path(subdir_template_path, 'mlpstamps.json').touch()  # creates file
...
def test_should_find_existing_mlpstamps(
    ...
    """Find `mlpstamps.json` in sub folder created by `cloned_mlpstamps_path`."""
...
def test_local_repo_typo(template, user_config_data, cloned_mlpstamps_path) -> None:
    """Wrong pointing to `mlpstamps.json` sub-directory should raise."""
    ...
    wrong_full_mlpstamps_path = os.path.join(
        os.path.dirname(cloned_mlpstamps_path), 'wrong-dir'
    )

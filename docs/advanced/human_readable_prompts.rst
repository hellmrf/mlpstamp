...
See the following mlpstamps config as example:
...
.. code-block:: json

    {
        ...
        "module_name": "{{ mlpstamps.package_name.replace('-', '_') }}",
        "package_name_stylized": "{{ mlpstamps.module_name.replace('_', ' ').capitalize() }}",

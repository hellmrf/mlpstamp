...
you will get the following user input when running mlpstamps::

  run_as_docker [True]:

User input will be parsed by :func:`~mlpstamps.prompt.read_user_yes_no`. The
following values are considered as valid user input:

    - ``True`` values: "1", "true", "t", "yes", "y", "on"
    - ``False`` values: "0", "false", "f", "no", "n", "off"

The above ``run_as_docker`` boolean variable creates ``mlpstamps.run_as_docker``,
which can be used like this::

  {%- if mlpstamps.run_as_docker -%}
...
mlpstamps is using `Jinja2's if conditional expression <https://jinja.palletsprojects

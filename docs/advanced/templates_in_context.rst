...
The values (but not the keys!) of `mlpstamps.json` are also Jinja2 templates.
...
Here is a `mlpstamps.json` with templated values for this pattern:

.. code-block:: JSON

    {
      "project_name": "My New Project",
      "project_slug": "{{ mlpstamps.project_name|lower|replace(' ', '-') }}",
      "pkg_name": "{{ mlpstamps.project_slug|replace('-', '') }}"
    }
